...
matmul:
    # Error checks
    li   t0 1
    blt  a1, t0, error       # validation 1
    blt  a2, t0, error       # validation 1
    blt  a4, t0, error       # validation 2
    blt  a5, t0, error       # validation 2
    bne  a2, a4, error       # validation 3
...
outer_loop_start:
    ...
    li   s1, 0
    mv   s4, a3              # s4 is address of second matrix
    blt  s0, a1, inner_loop_start  # if s0 < a1, goto inner_loop_start
...
inner_loop_end:
    ...
    addi s0, s0, 1           # s0(col) += 1
    slli t1, a2, 2
    add  s3, s3, t1
    blt  s0, a1, outer_loop_start  # if s0 < a1, goto outer_loop_start
